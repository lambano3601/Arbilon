...
class ArbitrageBot:
        ...
        for exchange_name in self.exchange_manager.exchanges.keys():
            try:
                ...
                if tokens_with_balance:
                    ...
                    tokens_with_balance.sort(key=lambda x: x['total'], reverse=True)
                    
                    for token in tokens_with_balance:
                        text += f"  • {token['currency']}: `{token['total']:.8g}` (Free: {token['free']:.8g})\n"
                else:
                    text += "  _No balances_\n"
    ...
    async def process_api_keys(self, update: Update, state: dict):
        ...
        try:
            ...
            # Pass the ENCRYPTED credentials (not plain text!)
            success, error_msg = await self.exchange_manager.add_exchange(
                exchange_name,
                api_keys['exchanges'][exchange_name]['api_key'],  # encrypted
                api_keys['exchanges'][exchange_name]['secret'],   # encrypted
                api_keys['exchanges'][exchange_name].get('passphrase')  # encrypted or None
            )
